function setUpDatabaseCallbacks() {
    ...
    window.electron.getItemByIDResponse((event, data) => setItemTitle(JSON.parse(data)));
    window.electron.getCategoriesForWorldResponse((event, data) => handleGetCategoriesResponse(JSON.parse(data)));
    window.electron.getNotesForItemResponse((event, data) => populateNotesList(JSON.parse(data)));
